...
int main(int argc, char* argv[]){
    ...
    if(argc > 6){
        LIMIT2 = (int)strtol(argv[4],NULL,10);
        THREADS = (int)strtol(argv[5],NULL,10);
        BSIZE = (int)strtol(argv[6],NULL,10);
    }
    printf("running ./vtCuda.out with %d %d %d %d %d %d\n",N,d,searchNeis,LIMIT2,THREADS,BSIZE);
